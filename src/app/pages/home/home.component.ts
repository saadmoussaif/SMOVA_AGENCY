...
type SectorIcon = 'rocket' | 'heart-pulse' | 'building-2' | 'store' | 'graduation-cap' | 'landmark' | 'plane' | 'factory';
type SectorItem = { icon: SectorIcon; title: string; text: string; detail: string; image: string; alt: string; };
type StackIcon = 'brain' | 'code-2' | 'database' | 'globe-2' | 'shopping-bag' | 'chart-no-axes-column' | 'palette' | 'video' | 'share-2' | 'smartphone' | 'cloud' | 'settings';
type StackItem = { icon: StackIcon; title: string; tools: string; detail: string; };
type ContactForm = { prenom: string; email: string; societe: string; countryIso: string; countryCode: string; tel: string; sujet: string; message: string; };
type ContactCountry = { iso: string; code: string; name: string; flagUrl: string; };
type ClientLogo = { name: string; type: 'image' | 'text'; logoUrl?: string; };
...
export class HomeComponent implements AfterViewInit, OnDestroy {
  ...
  // ✅ TranslationService injecté — accessible dans le template via i18n
  ...
  // ✅ Les phrases viennent du service de traduction
  ...
  readonly sectors: SectorItem[] = [
    { icon: 'rocket',        title: 'SaaS & startups',        text: 'MVP, scale-up, onboarding et outils internes pour equipes ambitieuses.',             detail: 'Nous concevons des produits SaaS avec parcours d inscription, dashboard, roles utilisateurs, paiement, analytics et architecture prete a evoluer.',               image: 'https://images.unsplash.com/photo-1551434678-e076c223a692?auto=format&fit=crop&w=900&q=80', alt: 'Equipe startup travaillant sur un produit digital' },
    { icon: 'heart-pulse',   title: 'Sante & bien-etre',      text: 'Experiences digitales fiables pour services sensibles, patients et praticiens.',      detail: 'Nous construisons des interfaces claires pour rendez-vous, dossiers, suivi patient, contenu medical et operations internes avec une attention forte a la confiance.', image: 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=900&q=80', alt: 'Professionnel de sante utilisant une interface digitale' },
    { icon: 'building-2',    title: 'Immobilier & PropTech',  text: 'Portails, CRM, parcours de reservation et outils pour agents immobiliers.',           detail: 'Nous aidons les agences et promoteurs avec portails annonces, estimation, espace client, visite virtuelle, CRM et workflows de qualification.',                      image: 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=900&q=80', alt: 'Maison moderne representant le secteur immobilier' },
    { icon: 'store',         title: 'Retail & e-commerce',    text: 'Boutiques, catalogues, paiement, logistique et automatisation commerciale.',           detail: 'Nous optimisons le tunnel d achat, les fiches produits, la gestion catalogue, les campagnes, le tracking et les automatisations apres-vente.',                        image: 'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=900&q=80', alt: 'Experience e-commerce et paiement en ligne' },
    { icon: 'graduation-cap',title: 'Education & EdTech',     text: 'Plateformes e-learning, espaces etudiants et outils de gestion pedagogique.',         detail: 'Nous creons des LMS, portails d inscription, espaces formateurs, contenus interactifs, tableaux de bord et automatisations pour ecoles, centres et organismes.',      image: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=900&q=80', alt: 'Etudiants travaillant ensemble avec des ordinateurs' },
    { icon: 'landmark',      title: 'Finance & FinTech',      text: 'Interfaces securisees pour services financiers, paiement et reporting.',               detail: 'Nous accompagnons les acteurs finance avec dashboards, onboarding client, parcours de paiement, reporting, automatisation documentaire et experiences rassurantes.',   image: 'https://images.unsplash.com/photo-1554224155-6726b3ff858f?auto=format&fit=crop&w=900&q=80', alt: 'Analyse financiere et documents de gestion' },
    { icon: 'plane',         title: 'Tourisme & Hotellerie',  text: 'Reservation, experience client, sites multilingues et acquisition directe.',           detail: 'Nous construisons des sites de reservation, moteurs de demande, parcours multilingues, contenus destination, CRM client et campagnes pour hotels, riads et voyages.',  image: 'https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=900&q=80', alt: 'Lieu touristique avec piscine et architecture de voyage' },
    { icon: 'factory',       title: 'Industrie & Services',   text: 'Digitalisation des operations, outils internes et suivi de performance.',              detail: 'Nous developpons des extranets, outils de suivi, formulaires terrain, dashboards de production, automatisations administratives et integrations avec vos systemes.',     image: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=900&q=80', alt: 'Equipe industrielle travaillant avec des technologies' },
  ];
  ...
  selectContactCountry(countryIso: string): void {
    const c = this.countries.find(c => c.iso === countryIso);
    if (!c) return;
    this.form.countryIso  = c.iso;
  ...
  onSubmit(event: Event): void {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        this.submitted = false;
        this.form = { prenom: '', email: '', societe: '', countryIso: 'MA', countryCode: '+212', tel: '', sujet: '', message: '' };
      }, 4000);
    }, 1800);
  }
  ...
  ngAfterViewInit(): void {
    ...
    gsap.from('.hero-typing-panel > *', { y: 38, opacity: 0, duration: 0.8, ease: 'power3.out', stagger: 0.08, delay: 0.25 });
    gsap.from('.reveal-item',           { y: 24, opacity: 0, duration: 0.7, ease: 'power2.out', stagger: 0.08, delay: 0.55 });

    if (loader) {
      gsap.to(loader, { opacity: 0, yPercent: -100, duration: 0.8, ease: 'power2.inOut', delay: 1.45, onComplete: () => loader.remove() });
    }
  }
  ...
  private runHeroTyping(): void {
    ...
    const current = phrases[this.heroPhraseIndex];

    this.isDeletingHeroText ? this.heroLetterIndex-- : this.heroLetterIndex++;
    this.displayedHeroText = current.slice(0, this.heroLetterIndex);

    let delay = this.isDeletingHeroText ? 34 : 58;

    if (!this.isDeletingHeroText && this.heroLetterIndex === current.length) {
      delay = 1450; this.isDeletingHeroText = true;
    }
  ...
  previousSector(): void {
    this.selectedSectorIndex = this.selectedSectorIndex === 0 ? this.sectors.length - 1 : this.selectedSectorIndex - 1;
    this.expandedSectorIndex = null;
  }

  nextSector(): void {
    this.selectedSectorIndex = this.selectedSectorIndex === this.sectors.length - 1 ? 0 : this.selectedSectorIndex + 1;
    this.expandedSectorIndex = null;
  }
